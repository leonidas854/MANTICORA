...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  void _setQuery(void Function(LibraryQuery) _) {}
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: CustomScrollView(
          slivers: [
            if (query.folderId != null) _breadcrumb(query),
            folders.when(
            ...
            docs.when(
              ...
              data: (list) {
                if (list.isEmpty) return SliverFillRemaining(child: _empty(query));
  ...
  PreferredSizeWidget _normalBar(LibraryQuery query) => AppBar(
        ...
        actions: [
          ...
          IconButton(
            tooltip: 'Vista',
            icon: Icon(ref.watch(settingsProvider).gridView
  ...
  Widget _empty(LibraryQuery query) {
    ...
    return EmptyState(
      ...
      subtitle: 'Pulsa el boton de la camara para escanear tu primer documento.',
    ...
    switch (value) {
      ...
      case 'folder':
        final name = await promptText(context,
            title: 'Nueva carpeta', label: 'Nombre', confirmLabel: 'Crear');
        if (name != null) {
  ...
  Future<void> _folderMenu(Folder folder) async {
    ...
    if (action == 'rename') {
      final name = await promptText(context,
          title: 'Renombrar carpeta', initial: folder.name);
  ...
  Future<void> _moveSelection() async {
    ...
    for (final id in _selection) {
      await repo.setFolder(id, target.isEmpty ? null : target);
    }
    setState(_selection.clear);
  }
  ...
  Future<void> _mergeSelection() async {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => PdfToolsScreen(preselectedDocumentIds: ids)),
    );
  }
}
